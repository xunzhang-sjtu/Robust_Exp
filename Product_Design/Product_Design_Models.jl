function Product_Design_Ours_ETO(d,nu0, nu, r0, r, C_mat)
...
function Product_Design_ETO(d,nu0, nu, r0,r, C_mat)
    model = Model(Mosek.Optimizer)
    set_attribute(model, "QUIET", true)
    # 定义变量
    @variable(model, a0 >= 0)                       
...
function Robust_Product_Design(N, n_x, n_c, gamma, psi_lb, psi_ub, phi_lb, phi_ub, alp0, alp, beta, A, r0, r, z_input,C_mat)
    ...
    # @variable(model, 0.0 <= eta[1])
    # @constraint(model, [eta[1];omega_0[1];omega[1,:];pi_[1,:];vec(D_[1,:,:])] in MOI.NormOneCone())
    # @variable(model, 0.0 <= eta[2])
    # @constraint(model, [eta[2];omega_0[2];omega[2,:];pi_[2,:];vec(D_[2,:,:])] in MOI.NormOneCone())
...
function Robust_Product_Design_Condense_Parameters(N, n_x, gamma, psi_lb, psi_ub, phi_lb, phi_ub, nu0, nu, r0, r)
